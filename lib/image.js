...
function Image (imageType, bufferType) {
    ...
    this.encode = function (buf, width, height, quality, callback) {
        var encoder;
        if(imageType == 'jpeg') {
            encoder = new libs[imageType][encoders[imageType]](buf, width, height, quality, bufferType);

        } else {
            encoder = new libs[imageType][encoders[imageType]](buf, width, height, bufferType);
        }

        encoder.encode(callback);
    }

    this.encodeSync = function (buf, width, height, quality) {
        var encoder;
        if(imageType == 'jpeg') {
            encoder = new libs[imageType][encoders[imageType]](buf, width, height, quality, bufferType);
        } else {
